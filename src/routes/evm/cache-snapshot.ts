// /cache/* — public read-only cache APIs. EVM port: replaces sns cache with
// ens cache, drops site/site-file cache types.
...
function dbPath(): string { return join(CACHE_DIR, "cache.db"); }
...
function isSearchQueryTooShort(q: string | null): boolean { return !!q && q.length < MIN_QUERY_CHARS; }
function isSearchQueryTooLong(raw: string | undefined): boolean { return !!raw && raw.length > MAX_QUERY_CHARS; }
...
function fromBase64Url(encoded: string): string | null {
  ...
  try { return Buffer.from(padded, "base64").toString("utf8"); } catch { return null; }
}
...
function ftsPhrase(raw: string): string { return `"${raw.replaceAll('"', '""')}"`; }
function hashKey(key: string): string { return createHash("sha256").update(key).digest("hex").slice(0, 16); }
...
function isInsideCacheDir(path: string): boolean { return isInsideRoot(CACHE_ROOT, path); }
async function realCacheRoot(): Promise<string | null> {
  try { return await realpath(CACHE_ROOT); } catch { return null; }
}
...
function contentTypeFor(row: CacheRow): string {
  ...
  if (["meta", "rows", "user", "signer-index", "ens"].includes(row.type)) return "application/json";
  if (["render", "view"].includes(row.type)) return "application/octet-stream";
  return "application/octet-stream";
}
function summarizeText(text: string, truncated: boolean) {
  try { return { kind: "json", value: JSON.parse(text), truncated }; }
  catch { return { kind: "text", text, truncated }; }
}
function summarizeValue(value: unknown, previewBytes = PREVIEW_BYTES) {
  ...
  let json: string;
  try { json = JSON.stringify(value) ?? "null"; } catch { json = String(value); }
  if (Buffer.byteLength(json) <= previewBytes) return { kind: "json", value, truncated: false };
...
async function previewForPath(path: string) {
  ...
  try {
    ...
    if (buffer.includes(0)) return { kind: "binary", bytes: stat.size, truncated };
...
cacheRouter.get("/info", (c) => {
  ...
  ).all()) byType[row.type] = { entries: row.n, bytes: row.bytes };
  db.close();
  return c.json({ entries: total?.n ?? 0, totalSize: total?.bytes ?? 0, byType });
});
...
cacheRouter.get("/entries", (c) => {
  ...
  if (isSearchQueryTooLong(c.req.query("q"))) return c.json({ error: "q is too long", maxChars: MAX_QUERY_CHARS }, 400);
  if (isSearchQueryTooShort(q)) return c.json({ error: "q is too short", minChars: MIN_QUERY_CHARS }, 400);
  if (type && !isCacheType(type)) return c.json({ error: "invalid cache type", types: CACHE_TYPES }, 400);
  if (c.req.query("cursor") && !cursor) return c.json({ error: "invalid cursor" }, 400);
  if (q && !hasExplorerSearchIndex()) return c.json({ error: "cache search index unavailable" }, 503);

  const db = openReadonlyDb();
  if (!db) return c.json({ entries: [], count: 0, limit, nextCursor: null });

  const where: string[] = [];
  const params: Array<string | number> = [];
  if (type) { where.push("ce.type = ?"); params.push(type); }
  if (q) { where.push("fts.key MATCH ?"); params.push(ftsPhrase(q)); }
  if (cursor) {
...
cacheRouter.get("/memory", (c) => {
  ...
  if (isSearchQueryTooLong(c.req.query("q"))) return c.json({ error: "q is too long", maxChars: MAX_QUERY_CHARS }, 400);
  if (isSearchQueryTooShort(q)) return c.json({ error: "q is too short", minChars: MIN_QUERY_CHARS }, 400);
  if (offset === null) return c.json({ error: "invalid cursor" }, 400);
...
// Stages blobs + a VACUUM-INTO cache.db into a tmp dir. Caller owns cleanup.
...
async function buildSnapshotStage(): Promise<{ stage: string } | { error: string }> {
  ...
  const setup = await runShell([
    ...
    `mkdir -p ${stage}`,
    `cd ${CACHE_DIR}`,
    // Exclude live WAL/SHM and backups/ (to avoid recursive packing)
...
// ─── POST /cache/backup — admin-only, writes snapshot to PV ─────────────────
// Atomic rename so a crashed backup never leaves a half-written file.
// Mutex prevents concurrent backups racing on the same .tmp path.

const BACKUP_DIR = resolve(process.env.BACKUP_DIR || join(CACHE_DIR, "backups"));
...
cacheRouter.post("/backup", async (c) => {
  ...
  if (backupRunning) return c.json({ error: "backup already in progress" }, 409);
  backupRunning = true;
  try {
    ...
    console.log(`[cache] backup written: ${BACKUP_FILE} (${info.size} bytes)`);
    return c.json({ ok: true, path: BACKUP_FILE, size: info.size, mtime: info.mtimeMs });
  } finally {
    backupRunning = false;
  }
});
...
cacheRouter.get("/backup", async (c) => {
  try {
    const info = await stat(BACKUP_FILE);
    return c.json({ exists: true, path: BACKUP_FILE, size: info.size, mtime: info.mtimeMs });
  } catch {
