...
export const openapiSpec = {
  ...
  paths: {
    ...
    "/media/{txHash}": { get: { tags: ["assets"], summary: "Passive image/audio/video bytes from an inscription row", description: "Accepts base64 data URLs in body/data, up to 8 MiB encoded. Rejects HTML/SVG/external URLs. Use network to select the EVM chain.", parameters: [txHash, { name: "network", in: "query", schema: { type: "string" } }, { name: "Range", in: "header", schema: { type: "string" } }], responses: { 200: { description: "Media bytes" }, 206: { description: "Partial media" }, 400: { description: "Invalid hash" }, 404: { description: "Missing inscription" }, 413: { description: "Too large" }, 415: { description: "Unsupported media" }, 416: { description: "Invalid range" }, 422: { description: "Invalid encoding" }, 502: { description: "Chain read failed" } } } },
