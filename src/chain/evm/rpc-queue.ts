// In-process job queue for RPC-heavy work.
//
// Replaces the old token-bucket limiter. Two priorities so interactive
// routes never sit behind background chain walks. Tunables exposed via
// POST /admin/queue — operators can adjust without restart.
...
export function getQueueConfig(): QueueConfig { return { ...config }; }
...
export function getQueueStats() {
  return {
    config: { ...config },
    pending: { interactive: interactiveQueue.length, background: backgroundQueue.length },
    inFlight,
    metrics: { ...metrics },
  };
}
